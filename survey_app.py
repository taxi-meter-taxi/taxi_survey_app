...
if not st.session_state["authenticated"]:
    ...
    if edit_id:
        ...
        with st.form("edit_entry"):
            ...
            q3 = st.radio("③ネット解析の評価", ["〇 良い", "× 悪い"], index=["〇 良い", "× 悪い"].index(entry["ネット解析の評価"]))
            q3_comment = st.text_input("ネット解析の評価コメント", value=entry["ネット解析の評価コメント"])
            q4 = st.radio("④ネット解析のコスト感", ["〇 安い", "△ 良い", "× 高い"], index=["〇 安い", "△ 良い", "× 高い"].index(entry["ネット解析のコスト感"]))
            q4_comment = st.text_input("ネット解析のコスト感コメント（目標価格、要望）", value=entry["ネット解析のコスト感コメント"])
            q5 = st.radio("⑤売上を上げたい事業者か？", ["〇 はい", "× いいえ"], index=["〇 はい", "× いいえ"].index(entry["売上を上げたい事業者か？"]))
            q5_comment = st.text_input("売上コメント", value=entry["売上コメント"])
            q6 = st.radio("⑥分析ツールを使っているか？", ["〇 はい", "× いいえ"], index=["〇 はい", "× いいえ"].index(entry["分析ツールを使っているか？"]))
            q6_comment = st.text_input("分析ツールコメント（使っているツール名）", value=entry["分析ツールコメント"])
            q7 = st.radio("⑦複数拠点、車庫が離れている", ["〇 はい", "× いいえ"], index=["〇 はい", "× いいえ"].index(entry["複数拠点・車庫が離れている"]))
            q7_comment = st.text_input("複数拠点コメント", value=entry["複数拠点コメント"])
            q8 = st.radio("⑧無線の使用率", ["〇 高い", "× 低い"], index=["〇 高い", "× 低い"].index(entry["無線の使用率"]))
            q8_comment = st.text_input("無線コメント（比率など）", value=entry["無線コメント"])
            q9 = st.radio("①インバウンド顧客が多いか？", ["〇 多い", "× 少ない"], index=["〇 多い", "× 少ない"].index(entry["インバウンド顧客が多いか？"]))
            q9_comment = st.text_input("インバウンドコメント", value=entry["インバウンドコメント"])
            q10 = st.radio("②流しか付け待ちどちらが多いか？", ["流し", "付け待ち"], index=["流し", "付け待ち"].index(entry["流しか付け待ちどちらが多いか？"]))
            q10_comment = st.text_input("流しコメント", value=entry["流しコメント"])
            q11 = st.radio("③乗務員の新規採用は多いか？", ["〇 多い", "× 少ない"], index=["〇 多い", "× 少ない"].index(entry["乗務員の新規採用は多いか？"]))
